...
Given the extensive body of work already done on this topic, it is fitting that a chapter be devoted to bringing the results of this study into this long standing conversation.  In order to simplify, I will focus on how my results compare with those of five closely related publications.  \citeauthor{SullMoengStev}'s \citeyear{SullMoengStev} \acs{LES} study was seminal and shed invaluable light on \acs{CBL} entrainment zone structure.  Whereas \citeauthor{BrooksFowler2}'s (\citeyear{BrooksFowler2}) work contains the most recent \acs{LES} results on the topic framed within an up to date review of \acs{CBL} height and \acs{EZ} definitions.  \citeauthor{FedConzMir04} (\citeyear{FedConzMir04}) provides an invaluable bridge between \acs{LES} and bulk models, while the closely related \acs{DNS} and analytical study of \citeauthor{GarciaMellado} (\citeyear{GarciaMellado}) clearly presented the two-layer \acs{EZ} concept and contributes to answering the question as to how fine a resolution is required to realistically capture \acs{CBL} growth and \acs{EZ} structure.  \citeauthor{SullPat} (\citeyear{SullPat}) address this last point, using an \acs{LES}, and was pivotal in guiding how the runs in this thesis were set up.  Finally \citeauthor{Sorbjan1} (\citeyear{Sorbjan1}) focused on the effects of upper lapse rate on the turbulence in the upper \acs{CBL} and provided ideas upon which I based aspects of this thesis.\\

Section \ref{sec:gensetup} addresses the need to for high vertical resolution for the steep gradients within the entrainment zone.  Drawing upon the results of \citeauthor{SullPat} (\citeyear{SullPat}) I then extend this to the possible need to resolve the steep horizontal gradients.  Mention of how \citeauthor{GarciaMellado} (\citeyear{GarciaMellado}) contributes to this question follows.  I present and compare certain results from the study described in this thesis in order to give this ongoing question sufficient consideration and before proceeding with applying the other results as a meaningfull contribution to the conversation.     I then touch upon how the domain size and initial conditions compare bearing in mind possible implications of the similarities and differences.\\

In Section \ref{sec:entzonestruc} I describe the problems encountered when using the gradient method, as well as the results obtained using my chosen method of determining local \acs{ML} height in the context of the results of \citeauthor{SullMoengStev}'s \citeyear{SullMoengStev} and \citeauthor{BrooksFowler2}'s (\citeyear{BrooksFowler2}).  The influence of $\gamma$ on the turbulent fluctuations of vertical velocity and potential temperature is discussed and compared with the results of \citeauthor{Sorbjan1} (\citeyear{Sorbjan1}) before addressing the dependence of the downward moving positive potential temperature fluctuation at $h$ on $\gamma$.  This then leads to an explaination of the potential temperature fluctuation scale $\delta h \gamma$.\\ 
